...
def create_tokenized_smiles_json(tokenizer,data_dir, split, config_output_name, label_filename, max_length):
    ...
    with open(os.path.join(data_dir, label_filename), "r") as f:   # 自己准备一个label.smi
        for i, l in enumerate(tqdm(f)):
            try:
                ...
                #encodingids = [start_token_id] + encodingids[:-1]
                cap_len = max_length
            ...
            current_sample = {"filepath": data_dir, "filename": "{}".format(idx), "imgid": 0, "split": split,
                              "sentences": [{"MorganFingerprint": fp, "raw": smiles, "label": label,"tokens": encoding.tokens,  "ids": encodingids , "length": cap_len}]}  # note if image augmentation ever happens need to introduce a sentence id token. see mscoco json for example
...
def main(args):
    ...
    print("Creating JSON")
    create_tokenized_smiles_json(tokenizer,args.data_dir, args.data_split, args.config_output_name, args.label_filename, args.max_length)
    print("JSON created")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tokenizer', default='/tokenizers/tokenizer_vocab_100.json', type=str, help='tokenizer name in the folder tokenizers/')
    parser.add_argument('--test_string', type=str, default='C1C2=CC=CC=C2C3=CC4=CC=CC=C4C=C31', help='a SMILES string to test tokenizer with')
    parser.add_argument('--data_dir', default='./train_C', type=str, help='directory of data to be processed. Expect a labels.smi file and associated images')
    parser.add_argument('--data_split', default='train', type=str, help='name of the portion of data being processed. Typical names are training, validation, and evaluation.')
    ...
    parser.add_argument('--image_output_filename', default='training_C', type=str, help='prefix for output image, caption, and caption length files.')
