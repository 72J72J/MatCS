...
from encoder import Resnet101Encoder
import pandas as pd
...
def main(args):
    ...
    if ((not os.path.isfile(processed_data_file_train))):
        ...
        train_data = TestbedDataset(args.data_dir_h, 'training','training_C' , transforms.Compose([normalize]), root='./', dataset= dataset_train + '_data', xd=train_drugs, y=labels,
                                    smile_graph=train_smile_graph)

        val_data = TestbedDataset(args.data_dir_h, 'validation','validation_C' , transforms.Compose([normalize]) , root='./', dataset= dataset_val + '_data', xd=val_drugs, y=val_labels,
    ...
    val_loader = DataLoader(val_data, batch_size=args.batch_size, shuffle=True, pin_memory=True, drop_last=True)
    print(train_loader)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label_filename', type=str, default='labelszong.smi',
                        help='name of labels file in case dataset is enourmous')
    parser.add_argument('--data_dir_h', default='/data2/ydai/GCN/data/ZONG/', type=str,
    ...
    parser.add_argument('--model_path', default='./models/vocab', type=str, help='model path')
