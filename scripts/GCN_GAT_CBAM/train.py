...
def main(args):
    ...
    if ((not os.path.isfile(processed_data_file_train))):
        ...
        train_data = TestbedDataset(args.data_dir_h, 'training_H', 'training_C', transforms.Compose([normalize]),
                                    root='./', dataset=dataset_train + '_data', xd=train_drugs, y=labels,
                                    smile_graph=train_smile_graph)

        val_data = TestbedDataset(args.data_dir_h, 'validation_H', 'validation_C', transforms.Compose([normalize]),
