...
class CaptionDataset(Dataset):
    def __init__(self, data_folder, data_name,C_data_name, transform):
        ...
        with open(os.path.join(data_folder, 'smiles' + data_name + '.json'), 'r') as j:
            self.smiles = json.load(j)
    ...
    def __getitem__(self, i):
        ...
        smiles = torch.LongTensor([self.smiles[i]])
        data = (img1, caption, label, img2, smiles)
        return data
